...
GitHub の Issues 一覧（`github.com/<owner>/<repo>/issues` と `github.com/issues`）や
Issue・PR ページ内の Issue リンク（本文・コメント中の参照、sub-issues など）を
クリックしたとき、GitHub Projects のようなスライドインのサイドパネルで
...
- Issues 一覧・Issue / PR ページ内の Issue リンクをクリック → サイドパネルでプレビュー
- 本物の Issue ページに近い 2 カラム構成：本文・コメント（作者ヘッダー付きボックス）＋
  右サイドバー（Assignees / Labels / Type / Projects / Milestone / Fields）
- プロジェクトは **Status 付き**で表示（GitHub がプリロードするのは Status のみ。
  他のプロジェクトフィールドはペイロードに含まれないため表示不可）
- パネル内の Issue リンクもパネル内で開ける（ヘッダーの ← で戻る）
- 閉じる: `Esc` / 背景クリック / 右上の ✕
...
- **PR 自体はパネル対象外**（通常どおり遷移する）。GitHub が PR ページに会話データを
  プリロードしない構造のため、Issue と同じ方式ではプレビューできない
